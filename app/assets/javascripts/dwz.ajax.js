...
//open a new navTab
function openTabDone(form,navTabId){
         var $form = $(form);
        // alert("abc");
          navTab.openTab('resultCalculate','/groups/a',{title:"calculate result",fresh:false,data:{}});
	
        //add by chaungye start
        $.pdialog.closeCurrent();
        //add by chaungye end
	if (form[DWZ.pageInfo.pageNum]) form[DWZ.pageInfo.pageNum].value = 1;
          setTimeout(function(){navTab.reload($form.attr('action'), {data: $form.serializeArray()})},200);
...
{
  navTab.reload(json.url,{data:{matrix_config_id:json.config_id},navTabId:json.navTabId});
}
//add by chuangye end
